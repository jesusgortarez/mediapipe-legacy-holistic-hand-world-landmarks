...
with mp_holistic.Holistic(static_image_mode=True, model_complexity=1) as holistic, \
    ...
    # --- Resultados Holistic ---
    holistic_left = results_holistic.left_hand_world_landmarks
    holistic_right = results_holistic.right_hand_world_landmarks

    # --- Resultados Hands ---
    hands_left = None
    hands_right = None
    if results_hands.multi_hand_world_landmarks:
        for handedness, hand_world_landmarks in zip(results_hands.multi_handedness, results_hands.multi_hand_world_landmarks):
            ...
            if label == 'Left':
                hands_right = hand_world_landmarks
            elif label == 'Right':
                hands_left = hand_world_landmarks

    # Mostrar WORLD_LANDMARKS (Coordenadas métricas) comparadas
    print("=== COMPARACIÓN World landmarks ===")
